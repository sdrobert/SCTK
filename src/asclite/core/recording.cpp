...
Recording::~Recording()
{
	...
	map<string, class Filter*>::iterator fi, fe;
	...
	while(fi != fe)
	{
		class Filter* ptr_elt = fi->second;
...
void Recording::Load(const string& _references, const string& _refType, const vector<string> & _hypothesis_files, const vector<string> & _hypothesis_titles, const vector<string> & _hypothesis_types, const string& uemFile, const string& speakeralignfile)
{
    ...
    sprintf(buffer, "Load %zu hypothesis", _hypothesis_files.size());
...
void Recording::AlignGeneric()
{
	...
	while (segmentor->HasNext())
	{
		...
		if (!ignoreSegs)
		{
			...
			sprintf(buffer, "%zi GAS cost: (%d)", gas->GetNbOfGraphAlignedToken(), (dynamic_cast<Levenshtein*>(aligner_instance)->GetCost()));
			LOG_DEBUG(logger, buffer);
    ...
    while(i != ei)
    {
        ...
        while (segmentor->HasNext())
        {
            ...
            if (!ignoreSegs)
            {
                ...
                sprintf(buffer, "Store %zi GAS for hyp: '%s' cost: (%d)", gas->GetNbOfGraphAlignedToken(), hyp_name.c_str(), (dynamic_cast<Levenshtein*>(aligner_instance))->GetCost());
                LOG_DEBUG(logger, buffer);
