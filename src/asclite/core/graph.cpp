...
void Graph::SetDimension(const size_t& dim)
{
	...
	{
        char buffer [BUFFER_SIZE];
        sprintf(buffer, "Graph::SetDimension() - Invalid dimension (%zi)!", dim);
...
void Graph::SetDimensionDeep(const size_t& dim, const size_t& deep)
{
	...
	{
        char buffer [BUFFER_SIZE];
        sprintf(buffer, "Graph::SetDimensionDeep() - Invalid dimension (%zi), max: %zi", dim, m_Dimension);
		LOG_FATAL(logger, buffer);
		exit(E_INVALID);
	}
}
...
void Graph::SetIndexRef(const size_t& ind)
{
	...
	{
        char buffer [BUFFER_SIZE];
        sprintf(buffer, "Graph::SetIndexRef() - Invalid dimension (%zi), max: %zi", ind, m_Dimension);
		LOG_FATAL(logger, buffer);
		exit(E_INVALID);
	}
}
