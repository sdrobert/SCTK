...
# Version 0.21 April 13, 2021
#    - Append path elements to PATH with Config{path_sep} (for windows compat).
...
"                     Otherwise the default path is used. On windows, use semicolons\n".
"                     instead of colons.\n".
"      -o numSpkr\n".
...
sub get_version{
	...
	if ($buffer =~ /Version: v?(\d+\.\d+)[a-z]*/){
	    $ver = $1;
	} elsif ($buffer =~ /Version: v?(\d+)/){
	    $ver = $1;
	}
...
sub VerifyResources
{
	...
	if ($buffer =~ /sc_stats Version: (\d+\.\d+)[a-z]*,/){
	    $ver = $1;
	}
    ...
    $ver = &get_version($ACOMP,"acomp.pl");
...
sub RunScoring
{
	...
	if($produceAlignmentGraphs){
        ...
        mkdir "$hyp_oname.alignments";
	    $command = "$ALIGN2HTML $ali2htmOpt -a $hyp_oname.aligninfo.csv -o $hyp_oname.alignments";
	    print "   Exec: $command\n" if ($Vb);
	    $rtn = system($command);
